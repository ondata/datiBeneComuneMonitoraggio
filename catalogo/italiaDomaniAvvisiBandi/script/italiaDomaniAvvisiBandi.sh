...
mlr --json put -S '$dataAperturaDate = strftime(strptime($dataApertura, "%d/%m/%y"),"%Y-%m-%d");if($dataChiusura=~".*[0-9]{2}.*"){$dataChiusuraDate = strftime(strptime($dataChiusura, "%d/%m/%y"),"%Y-%m-%d")} else {$dataChiusuraDate=""}' then clean-whitespace then sort -r dataAperturaDate -f titolo then put '$URL=sub($URL,"^/content","https://italiadomani.gov.it/content")' "$folder"/../output/"$nome"_raw.json >"$folder"/../output/"$nome"_latest.json
...
if [ ! -f "$folder"/../output/"$nome"_archivio.json ]; then
  ...
  mlr --j2c unsparsify "$folder"/../output/"$nome"_archivio.json >"$folder"/../output/"$nome"_archivio.csv
fi
